...
`packaging/recipes/threadpoolpro/all/conanfile.py`). Scoped to what isn't obvious
...
```bash
find $HOME/staging/lib/cmake/ThreadPoolPro -name "*.cmake"
```

Expect `ThreadPoolProConfig.cmake`, `ThreadPoolProConfigVersion.cmake`,
`ThreadPoolProTargets.cmake` (plus a per-config
`ThreadPoolProTargets-<config>.cmake`) under `lib/cmake/ThreadPoolPro/`.
...
target_link_libraries(test PRIVATE ThreadPoolPro::ThreadPoolPro)
EOF
...
```bash
conan create packaging/recipes/threadpoolpro/all --version 1.0.0 \
...
`packaging/recipes/threadpoolpro/all/test_package/` against the freshly-built
package — a `ThreadPoolPro linked and constructed successfully.` line in
